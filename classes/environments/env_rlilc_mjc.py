...
def minjerk(qi:torch.Tensor, qf:torch.Tensor, duration:float, t:float) -> list[torch.Tensor, torch.Tensor, torch.Tensor]:
    """ Compute position, velocity, acceleration of joints of minjerk trajectory w.r.t. time t.
...
class Env_RILC(MujocoEnv, utils.EzPickle):
    """ Environment for RL with objective of a trajetory control using PD+ILC+G+RL.
        ...
        Args:
            ...
            fl_noILC (bool, optional): flag to not use ILC control. Defaults to False.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # array([0.001, 0.001, 0.001, 0.1  ])
        # self.model.dof_frictionloss[0]
        # 0.001
        # self.model.dof_frictionloss[0] = 222
        # self.model.dof_frictionloss
        # array([2.22e+02, 1.00e-03, 1.00e-03, 1.00e-01])
        # self.model.dof_frictionloss[0] = 0.001
        ...
        self.delta_q_task = torch.tensor(torch.pi/20)
    ...
    def _init_env_vars(self):
        ...
        self.qf = self.q_range_task*rand_number
    ...
    def _updateRobot(self, uRL:torch.Tensor, uILC:torch.Tensor) -> list[list[torch.Tensor],list[torch.Tensor], bool]:
        ...
        # compute reference
        r_, dr_, ddr_ = minjerk(qi = self.qi, qf = self.qf, duration = self.taskT, t = self.t)
    ...
    def _get_obs(self, ctrlNew:list[torch.Tensor]) -> torch.Tensor:
        ...
        r_f, dr_f, ddr_f = minjerk(qi = self.qi, qf = self.qf, duration = self.taskT, t = t_pol)
...
class Env_classic_RL(Env_RILC):
    ...
    def step(self, u:np.ndarray):
        ...
        # q_old = self.pin_rob.q.clone()
        # # Update self.robot.q attribute
        # self.pin_rob.setState(q=q)
