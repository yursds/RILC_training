...
def run_single_episode_disturbance(target_episodes=None, magnitude=0.5, mode_list=None, disturbance_type='impulse', target_episode=None):
            ...
            if state['ep'] in target_episodes:
                ...
                if disturbance_type == 'impulse':
                    if not state['applied']:
                        noise = rng.normal(loc=0.0, scale=magnitude, size=ctrl.size)
    ...
    try:
        ...
        for mode in mode_list:
            ...
            state['ep'] = -1
            state['applied'] = False
            print(f"Running with single-episode disturbance: {mode} (ep={target_episodes})")
            try:
    ...
    img_path = os.path.join(IMG_DIR, f'single_ep_disturb_ep{ep_tag}_mag{magnitude:.3f}.pdf')
    fig.savefig(img_path, dpi=200)
    print(f"Saved plot to {img_path}")

    # # --- Plot disturbance history for each target episode ---
    # for ep_num in sorted(episode_disturbances.keys()):
    #     distb = episode_disturbances[ep_num]
    #     per_act = episode_per_actuator.get(ep_num, [])
        
    #     # Plot disturbance norm vs time
    #     fig2, ax2 = plt.subplots(1, 1, figsize=(10, 4))
    #     ax2.plot(distb, marker='o', linewidth=2, markersize=6, color='tomato')
    #     ax2.set_title(f"Disturbo applicato - Episodio {ep_num} (mag={magnitude}, type={disturbance_type})")
    #     ax2.set_xlabel('Step in episode')
    #     ax2.set_ylabel('Noise norm (L2)')
    #     ax2.grid(True, alpha=0.3)
        
    #     disturb_path = os.path.join(IMG_DIR, f'single_ep_disturb_disturbance_only_ep{ep_num}_mag{magnitude:.3f}.png')
    #     fig2.tight_layout()
    #     fig2.savefig(disturb_path, dpi=200)
    #     print(f"Saved disturbance history plot to {disturb_path}")
    #     plt.close(fig2)
        
    #     # Also plot per-actuator if available and small
    #     if per_act and len(per_act[0]) <= 10:
    #         fig3, ax3 = plt.subplots(1, 1, figsize=(10, 4))
    #         arr = np.array(per_act)
    #         for a in range(arr.shape[1]):
    #             ax3.plot(arr[:, a], label=f'Actuator {a}', marker='o', markersize=4)
    #         ax3.set_title(f'Per-actuator disturbance - Episodio {ep_num}')
    #         ax3.set_xlabel('Step in episode')
    #         ax3.set_ylabel('Noise torque')
    #         ax3.legend(loc='upper right', ncol=min(3, arr.shape[1]), fontsize='small')
    #         ax3.grid(True, alpha=0.3)
            
    #         per_act_path = os.path.join(IMG_DIR, f'single_ep_disturb_per_act_ep{ep_num}_mag{magnitude:.3f}.png')
    #         fig3.tight_layout()
    #         fig3.savefig(per_act_path, dpi=200)
    #         print(f"Saved per-actuator disturbance plot to {per_act_path}")
    #         plt.close(fig3)
...
if __name__ == '__main__':
    # Example 1: Single episode disturbance (backward compatible)
    # run_single_episode_disturbance(target_episodes=5, magnitude=1, mode_list=["ILC", "NOILC", "RL", "RILC"], disturbance_type='continuous')
    
    # Example 2: Multiple episodes with disturbance
    t_ep = [n for n in range(0, 30)]
    run_single_episode_disturbance(target_episodes=t_ep, magnitude=0.05, mode_list=["ILC", "NOILC", "RL", "RILC"], disturbance_type='continuous')
    
    # Alternative syntax (single int still works):
    # run_single_episode_disturbance(target_episodes=5, magnitude=1, ...)
